...
export const PoPage = () => {
    ...
    const supplierLabelById = useMemo(() => {
        const map = new Map()
        suppLookup.forEach((supplier) => {
            const id = getLookupId(supplier)
            const label = getLookupLabel(supplier)
    ...
    const statusLabelById = useMemo(() => {
        const map = new Map()
        poStatusLookup.forEach((status) => {
            const id = getLookupId(status)
            const label = getLookupLabel(status)
            if (id !== null && label) map.set(String(id), label)
        })
        return map
    }, [poStatusLookup])
    ...
    const poDecisionMetrics = useMemo(() => {
        ...
        pos.forEach((po) => {
            ...
            const orderDateValue = po?.order_date || po?.orderDate || po?.createdAt || po?.date || po?.created_at || null
    ...
    const getPoNumber = (po) => po?.po_no || po?.po_number || po?.name || `PO-${po?.id || 'N/A'}`

    const getDateLabel = (po) => {
        const rawDate = po?.order_date || po?.orderDate || po?.createdAt || po?.date || po?.created_at
        if (!rawDate) return '-'
    ...
    return (
        <main className="ml-0 pt-0 px-8 pb-12">
                    ...
                    <table className="w-full text-left border-separate border-spacing-y-1">
                        <thead>
                            <tr className="text-on-surface-variant">
                                <th className="px-6 py-5 text-[10px] font-bold uppercase tracking-wider font-label">PO Number</th>
                        ...
                        {!isLoadingPos && paginatedPos.map((po, index) => {
                            ...
                            return (
                                <tr key={po?.id || po?.po_id || po?.po_no || `po-${index}`} className={`bg-slate-50/50 ${rowBg} hover:bg-slate-100/50 transition-colors group`}>
                                    <td className="px-6 py-4 rounded-l-xl text-sm font-bold text-primary">{getPoNumber(po)}</td>
                                    ...
                                    <td className="px-6 py-4 text-right rounded-r-xl">
                                        <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button
                                                ...
                                                onClick={() => navigate('/poform', { state: { mode: 'view', po } })}
                                            >
                                            ...
                                            <button
                                                ...
                                                onClick={() => navigate('/poform', { state: { mode: 'update', po } })}
                                            >
